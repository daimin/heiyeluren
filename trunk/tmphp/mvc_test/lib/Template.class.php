<?php
/**
 * Punny : PHP Bunny - 打造最简单易用的PHP MVC框架
 * 
...
class Template {
	...
	public function cache($file) {
		...
		. "(\[[a-zA-Z0-9_\-\.\"\'\[\]\$\x7f-\xff]+\])*)";
		$template = preg_replace("/\{(\\\$[a-zA-Z0-9_\[\]\'\"\$\.\x7f-\xff]+)\}/s", "<?=\$this->tplVarArr[\'\\1\']?>", $template);
		$template = preg_replace("/$varRegexp/es", "addquote('<?=\$this->tplVarArr[\'\\1\']?>')", $template);
		$template = preg_replace("/\<\?\=\<\?\=$varRegexp\?\>\?\>/es", "addquote('<?=\$this->tplVarArr[\'\\1\']?>')", $template);
		$template = str_ireplace('$this->tplVarArr[\'$', '$this->tplVarArr[\'', $template);
		$template = str_replace('{__Action__}', 'index.php/', $template);
		...
		. "\$this->tpl->getInstance()->check('$file', '$md5data', $expireTime);"
		. "?>\r\n$template";
